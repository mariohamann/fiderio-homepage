<template>
  <Editor :lines="12">
    <template #input>
      <pre :class="{ loading: loading }" tabindex="0"><code
        v-pre class="language-markdown"
><span class="token title important"><span class="token punctuation">###</span> Dynamic sample description</span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>script</span><span class="token punctuation">&gt;</span></span></pre>
    ...
    <template #output>
      ...
      <p :class="{loading: loading}" class="mb-2">
        The last update of the sample was at <span class="dynamic">{{ time }}</span>.
      </p>
...
// variable
const lastTime = ref(getTime())
const time = ref(getTime())
const sample = ref([{ age: 30, happy: true }])
